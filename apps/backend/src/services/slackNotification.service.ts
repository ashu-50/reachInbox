...
function dedupeKey(senderId: string, hourWindow: string): string {
...
export async function notifyRateLimitReached(
  ...
  try {
    ...
    await sendSlackDirectMessage(
      ...
      `:warning: Sender *${senderEmail}* just hit its hourly send limit. Remaining emails will resume next hour.`
    );
  } catch (err) {
