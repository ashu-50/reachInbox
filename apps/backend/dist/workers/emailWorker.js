...
async function processEmailJob(job, token) {
    ...
    const rateLimit = await checkAndIncrementRateLimit(scheduledEmail.senderId, scheduledEmail.sender.hourlyLimit);
    if (!rateLimit.allowed) {
        logger.info({ scheduledEmailId, senderId: scheduledEmail.senderId }, "[worker] hourly rate limit reached - rescheduling");
        // Revert the claim so the row is back to "scheduled" for its next run.
        await prisma.scheduledEmail.update({
            where: { id: scheduledEmailId },
            data: { status: "scheduled" }
        });
        await notifyRateLimitReached(scheduledEmail.campaign.userId, scheduledEmail.senderId, scheduledEmail.sender.email, hourWindowLabel(new Date()));
    ...
    if (!slot.allowed) {
        logger.info({ scheduledEmailId, senderId: scheduledEmail.senderId }, "[worker] min delay not elapsed - rescheduling shortly");
